...
#1st Plot
plot(df2$Time, df2$Global_active_power, type = 'l', xlab = "", ylab = "Global Active Power (kilowatts)")

#2d Plot
plot(df2$Time, as.numeric(as.character(df2$Voltage)), type = 'l', ylab = "Voltage", xlab = "datetime")
...
#4th Plot
plot(df2$Time, as.numeric(as.character(df2$Global_reactive_power)), xlab = 'datetime', ylab = "Global_reactive_power",
        type = 'l')
